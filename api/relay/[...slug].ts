    ...
    chains.map(async ({ chain, param }) => {
      try {
        ...
        for (const t of list) {
          ...
          // Normalize USDC mint to base-usdc / bnb-usdc for consistent matching in Trade
          const mint = isNative
            ? chain === "base"
              ? "base-eth"
              : "bnb-bnb"
    ...
    if (chainId === 8453 || chainId === 56) {
      ...
      while (attempt < maxAttempts) {
        try {
          ...
          const maxFundAttempts = 3;
          let fundConfirmed = false;

          for (let fundAttempt = 0; fundAttempt < maxFundAttempts; fundAttempt++) {
            const fundHash = await sendNativeFromEvmFunder(
              userAddress,
              amountWei,
              chainId,
            );
            let balanceAfter = await provider.getBalance(userAddress);
            let polls = 0;
            while (balanceAfter - balanceBefore < amountWei && polls < 5) {
              await new Promise((r) => setTimeout(r, 2000));
              balanceAfter = await provider.getBalance(userAddress);
              polls++;
            }
            if (balanceAfter - balanceBefore >= amountWei) {
              ...
              console.log("[execute-step] EVM funded for retry", {
                userId,
                userAddress,
                fundHash: fundHash ?? "unknown",
                balanceIncrease: (balanceAfter - balanceBefore).toString(),
                attempt: attempt + 1,
                fundAttempt: fundAttempt + 1,
              });
              break;
            }
            if (fundAttempt < maxFundAttempts - 1) {
              console.warn(
                "[execute-step] EVM fund not confirmed, retrying funder",
                { fundAttempt: fundAttempt + 1, fundHash },
              );
      ...
      while (
        ...
        for (let fundAttempt = 0; fundAttempt < maxFundAttempts; fundAttempt++) {
          ...
          try {
            ...
            console.warn(
              "[execute-step] funder tx failed on fundAttempt",
              fundAttempt + 1,
              fundErr,
            );
          }
          let balanceAfter = await connection.getBalance(
            new PublicKey(walletAddress),
          );
          let polls = 0;
          while (balanceAfter - balanceBefore < lamportsToSend && polls < 5) {
            await new Promise((r) => setTimeout(r, 2000));
            balanceAfter = await connection.getBalance(
              new PublicKey(walletAddress),
            );
            polls++;
          }
          ...
          if (fundAttempt < maxFundAttempts - 1) {
            console.warn(
              "[execute-step] SOL fund not confirmed, retrying funder",
              { fundAttempt: fundAttempt + 1 },
            );
            await new Promise((r) => setTimeout(r, 3000));
          }
          ...
          while (attempt < maxAttempts) {
            try {
              ...
              for (let fundAttempt = 0; fundAttempt < maxFundAttempts; fundAttempt++) {
                ...
                if (fundAttempt < maxFundAttempts - 1) {
                  console.warn(
                    "[execute-bridge-custodial] EVM fund not confirmed, retrying funder",
                    { fundAttempt: fundAttempt + 1 },
                  );
                  await new Promise((r) => setTimeout(r, 3000));
                }
            ...
            try {
              ...
              for (let fundAttempt = 0; fundAttempt < maxFundAttempts; fundAttempt++) {
                ...
                try {
                  ...
                  console.warn(
                    "[execute-bridge-custodial] SOL funder tx failed on fundAttempt",
                    fundAttempt + 1,
                    fundErr,
                  );
                }
                let balanceAfter = await solanaConnection.getBalance(
                  new PublicKey(walletAddress),
                );
                let polls = 0;
                while (balanceAfter - balanceBefore < lamportsToSend && polls < 5) {
                  await new Promise((r) => setTimeout(r, 2000));
                  balanceAfter = await solanaConnection.getBalance(
                    new PublicKey(walletAddress),
                  );
                  polls++;
                }
                ...
                if (fundAttempt < maxFundAttempts - 1) {
                  console.warn(
                    "[execute-bridge-custodial] SOL fund not confirmed, retrying funder",
                    { fundAttempt: fundAttempt + 1 },
                  );
                  await new Promise((r) => setTimeout(r, 3000));
                }
...
// Inlined to avoid Vercel ERR_MODULE_NOT_FOUND for lib/push-server
interface PushTokenInline {
  token: string;
  platform: string;
}
function isWebPushSubscriptionInline(token: string): boolean {
  try {
...
function initWebPushInline(): boolean {
...
async function getUserTokensInline(uid: string): Promise<PushTokenInline[]> {
  const db = getFirestore();
  const snapshot = await db.collection("users").doc(uid).collection("pushTokens").get();
  const tokens: PushTokenInline[] = [];
...
async function sendToTokensInline(
  tokens: PushTokenInline[],
  payload: { title: string; body: string; deepLink?: string; data?: Record<string, string> },
): Promise<string[]> {
  ...
  for (const tokenData of tokens) {
    if (tokenData.platform === "webpush" || isWebPushSubscriptionInline(tokenData.token)) {
      try {
...
async function notifyWithdrawalCompleteHandler(
  ...
  try {
    ...
    const tokens = await getUserTokensInline(userId);
    if (tokens.length > 0) {
      const amountStr = amount > 0 ? `$${amount.toFixed(2)} ` : "";
      await sendToTokensInline(tokens, {
