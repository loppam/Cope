...
export function CopeWallet() {
  ...
  const [searchTypeLabel, setSearchTypeLabel] = useState<'Wallet' | 'X Account'>('Wallet');

  const detectSearchType = (input: string): 'wallet' | 'xhandle' => {
    const trimmed = input.trim();
    if (trimmed.startsWith('@') || (!trimmed.includes(' ') && trimmed.length < 20 && !trimmed.match(/^[A-Za-z0-9]{32,44}$/))) {
      return 'xhandle';
    }
    return 'wallet';
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#000000] to-[#0B3D2E]">
      ...
      <div className="p-4 sm:p-6 max-w-[720px] mx-auto">
        ...
        <div className="space-y-6">
          <div>
            <label className="block text-sm font-medium mb-2">Search by Wallet or X Account</label>
            <Input
              ...
              onChange={(e) => {
                ...
                setSearchTypeLabel(detected === 'wallet' ? 'Wallet' : 'X Account');
              }}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  handleSearch();
