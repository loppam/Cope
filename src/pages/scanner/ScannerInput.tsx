...
export function ScannerInput() {
  ...
  const handleScan = () => {
    if (canScan) {
      navigate('/scanner/loading', { state: { mints: validMints, lookback, minMatches, minTrades } });
    }
  };
      ...
      <div className="p-4 sm:p-6 max-w-[720px] mx-auto">
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              {mints.map((mint, index) => (
                <div key={index} className="flex gap-2">
                  ...
                  />
                  {mints.length > 2 && (
                    <Button variant="outline" size="sm" onClick={() => removeMintRow(index)}>
                      <X className="w-4 h-4" />
                    </Button>
                  )}
                </div>
            ...
            {mints.length < 10 && (
              <Button variant="ghost" onClick={addMintRow} className="mt-2 h-10 px-0">
          ...
          {/* Lookback Period */}
          <div>
            <label className="block text-sm font-medium mb-3">Lookback Period</label>
            <div className="flex gap-2">
              {['7D', '30D', '90D'].map((period) => (
                <button
                  key={period}
                  onClick={() => setLookback(period)}
                  className={`flex-1 h-10 rounded-[12px] text-sm font-medium transition-all ${
                    lookback === period
          ...
          <div className="space-y-3 pt-4">
            <Button onClick={handleScan} disabled={!canScan} className="w-full h-12">
              Scan Wallets
            </Button>
          </div>
